...
* @param {Number} [opts.lambda=1] - scale paramater
...
function mgf( t, options ) {
	...
	if ( isNumber( t ) ) {
		return mgf1( t, opts.lambda, opts.k );
	}
	if ( isMatrixLike( t ) ) {
		if ( opts.copy !== false ) {
		...
		return mgf5( out, t, opts.lambda, opts.k );
	}
	if ( isTypedArrayLike( t ) ) {
		...
		return mgf6( out, t, opts.lambda, opts.k );
	}
	if ( isArrayLike( t ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return mgf4( t, opts.lambda, opts.k, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return mgf3( out, t, opts.lambda, opts.k, opts.accessor );
		}
		return mgf2( out, t, opts.lambda, opts.k );
	}
	return NaN;
} // end FUNCTION mgf()
